...
async function testEndpoint(name, url, options, expectedStatus = 200) {
  ...
  if (result.ok && result.status === expectedStatus) {
    ...
    logTest(name, 'FAIL', `Status: ${result.status}, Expected: ${expectedStatus}`);
...
async function runTests() {
  ...
  if (supportResult.success) results.passed++; else results.failed++;
  ...
  if (helpFormResult.success) results.passed++; else results.failed++;
  ...
  if (queryResult.success) results.passed++; else results.failed++;
  ...
  if (supportResult.success && supportResult.data.caseId) {
    ...
    if (retrieveResult.success) results.passed++; else results.failed++;
  }
  ...
  const notFoundResponse = await makeRequest(`${BASE_URL}/nonexistent-endpoint`, { method: 'GET' });
  
  if (notFoundResponse.status === 404) {
    logTest('404 Handling', 'PASS', `Status: 404`);
    results.passed++;
  } else {
    logTest('404 Handling', 'FAIL', `Status: ${notFoundResponse.status}, Expected: 404`);
    results.failed++;
  }
  ...
  if (results.failed === 0) {
    ...
    log(`\n${colors.bright}⚠️  Some tests failed. Please check the output above.${colors.reset}`, 'yellow');
  }
