...
async function main() {
    ...
    for (const file of mdxFiles) {
        ...
        visit(tree, (node) => {
            if (node.type === 'heading') {
                ...
                currentSection = node.children.map((c) => c.value).join(' ');
                currentText = '';
            }
    ...
    const embeddedChunks = await Promise.all(mdxChunks.map(chunk => limit(async () => {
        try {
    ...
    const validChunks = embeddedChunks.filter(Boolean);
