...
async function deleteIndex() {
  ...
  if (!res.ok && res.status !== 404) {
    throw new Error(`Failed to delete index: ${res.status} ${await res.text()}`);
  }
...
async function createIndex() {
  ...
  if (!res.ok) {
    throw new Error(`Failed to create index: ${res.status} ${await res.text()}`);
  }
...
async function upsertChunks() {
  ...
  if (!Array.isArray(chunks)) throw new Error('lesson-chunks.json is not an array');
  ...
  for (let i = 0; i < chunks.length; i += 50) {
    ...
    if (vectors.some(v => !v.values.length)) {
      throw new Error('Some chunks are missing embedding values. Run embedding first.');
    }
