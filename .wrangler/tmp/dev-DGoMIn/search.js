...
var AIService = class {
  ...
  enhanceMatches(matches, query) {
    ...
    return matches
      .map((m) => ({
        ...m,
        score: (m.score || 1) * (docTypeBoost[m.metadata?.docType] || 1),
      }))
    ...
    switch (intent) {
      case "pricing": {
        ...
        const contextText = matches
          .map(
            (m) => m.metadata?.description || m.metadata?.text || m.text || "",
          )
          .join("\n");
...
Respond in Markdown only. Do not use HTML tags.`;
  ...
  if (intent === "general") {
    let paymentsMatch = matches.find(
      (m) => m.metadata?.url === "/lessons/payments",
    );
    let top =
      paymentsMatch || matches.find((m) => m.metadata?.url || m.metadata?.slug);
...
var routes = {
  ...
  "/support-case/": {
    handler: /* @__PURE__ */ __name((req, env, path) => {
