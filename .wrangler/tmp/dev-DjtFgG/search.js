...
// .wrangler/tmp/bundle-IueMZI/checked-fetch.js
...
// .wrangler/tmp/bundle-IueMZI/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-IueMZI/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-IueMZI/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
    ...
    if (path.startsWith("/query")) {
      try {
        ...
        if (!Array.isArray(matches)) {
          console.error(
            "Unexpected VECTORIZE.query response:",
            JSON.stringify(vectorizeResult),
          );
          return withCors(
            new Response(
              JSON.stringify({
                error: "VECTORIZE.query did not return an array of matches",
                details: vectorizeResult,
              }),
              { status: 500 },
            ),
          );
        ...
        matches.forEach((match) => {
          ...
          if (
            ...
            queryWords.some((word) => section.includes(word))
          ) {
            match.score += 1;
          }
        });
        ...
        return withCors(
          new Response(
            JSON.stringify({
              error: "Worker exception in /query",
    ...
    if (path.startsWith("/chat")) {
      try {
        ...
        const context = matches
          .map(
            (m2, i2) =>
              `${i2 + 1}. ${m2.metadata?.description || m2.metadata?.text || m2.text || ""}`,
          )
          .join("\n");
        const prompt = `
...
Respond in Markdown only. Do not use HTML tags.`;
        ...
        return withCors(
          new Response(
            JSON.stringify({
              error: "Worker exception in /chat",
              details: err instanceof Error ? err.message : err,
            }),
            { status: 500 },
          ),
