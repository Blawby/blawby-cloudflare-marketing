...
// .wrangler/tmp/bundle-pP0mlJ/checked-fetch.js
...
// .wrangler/tmp/bundle-pP0mlJ/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-pP0mlJ/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
    ...
    return `
      <div style="${baseStyles}">
        ...
        <div style="${contentStyles}">
          ...
          <div style="${messageBoxStyles}">${message}</div>
        </div>
        <div style="${footerStyles}">
          &copy; ${currentYear} Blawby. All rights reserved.
        </div>
      </div>
    `;
  } else {
...
async function sendEmail(resendApiKey, options) {
    ...
    if (path.startsWith("/chat")) {
      try {
        ...
        if (isPricingQuery) {
          ...
          if (/no setup fees?/i.test(contextText)) {
            setupFee = "No setup fees";
            foundAny = true;
          }
          if (/no hidden fees?/i.test(contextText)) {
            hiddenFee = "No hidden fees";
            foundAny = true;
          }
          let pricingLines = [];
          if (monthlyFee)
...
Best,
The Blawby Team`;
        const confirmResp = await sendEmail(resendApiKey, {
          from: "support@blawby.com",
          to: [email],
