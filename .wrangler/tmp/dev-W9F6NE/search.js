...
// .wrangler/tmp/bundle-N7ccBi/checked-fetch.js
...
// .wrangler/tmp/bundle-N7ccBi/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-N7ccBi/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var jsonResponse = /* @__PURE__ */ __name((data, status = 200) => withCors(Response.json(data, { status })), "jsonResponse");
var errorResponse = /* @__PURE__ */ __name((error, details, status = 500) => jsonResponse({ error, ...details && { details } }, status), "errorResponse");
var getEmbedding = /* @__PURE__ */ __name(async (ai, text) => {
  const embedding = await ai.run("@cf/baai/bge-small-en-v1.5", { text });
  return embedding.data[0];
}, "getEmbedding");
var queryVectorDB = /* @__PURE__ */ __name(async (env, queryVector, topK = 5) => {
...
var enhanceMatches = /* @__PURE__ */ __name((matches, query) => {
  ...
  return matches.sort((a2, b) => b.score - a2.score);
}, "enhanceMatches");
var sendEmail = /* @__PURE__ */ __name(async (apiKey, from, to, subject, text, html) => {
...
var createEmailTemplate = /* @__PURE__ */ __name((logoUrl, title, content) => `
...
`, "createEmailTemplate");
var handleQuery = /* @__PURE__ */ __name(async (request, env) => {
...
var handleUpsert = /* @__PURE__ */ __name(async (request, env) => {
...
var handleChat = /* @__PURE__ */ __name(async (request, env) => {
  const { query } = await request.json();
  const ai = new W(env.AI);
  const queryVector = await getEmbedding(ai, query.trim());
  ...
  if (isPricingQuery) {
    const contextText = matches.map((m2) => m2.metadata?.description || m2.metadata?.text || m2.text || "").join("\n");
    const feePatterns = [
      { pattern: /\$([0-9]+(?:\.[0-9]{2})?)\s*\/\s*month\s*\/\s*user/i, format: /* @__PURE__ */ __name((m2) => `**Monthly user license:** $${m2[1]} per user per month`, "format") },
      { pattern: /([0-9]+(?:\.[0-9]+)?)%\s*\+\s*([0-9]+¢|\$[0-9]+(?:\.[0-9]{2})?)\s*per.*card/i, format: /* @__PURE__ */ __name((m2) => `**Card payments:** ${m2[1]}% + ${m2[2]} per card transaction`, "format") },
      { pattern: /([0-9]+(?:\.[0-9]+)?)%.*ACH.*\(\$([0-9]+) cap\)/i, format: /* @__PURE__ */ __name((m2) => `**ACH/bank payments:** ${m2[1]}% per ACH (max $${m2[2]})`, "format") },
      { pattern: /([0-9]+(?:\.[0-9]+)?)%.*platform fee/i, format: /* @__PURE__ */ __name((m2) => `**Platform fee:** ${m2[1]}% platform fee (billed monthly)`, "format") },
      { pattern: /\$([0-9]+) fee for disputed payments.*chargebacks?/i, format: /* @__PURE__ */ __name((m2) => `**Chargeback fee:** $${m2[1]} per chargeback`, "format") }
    ];
...
For full details and the latest updates, [see our pricing page](/pricing).`;
  ...
  const context = matches.map((m2, i2) => `${i2 + 1}. ${m2.metadata?.description || m2.metadata?.text || m2.text || ""}`).join("\n");
  const prompt = `
...
Respond in Markdown only. Do not use HTML tags.`;
  ...
  const message = llmResponse?.response || "Sorry, I couldn't find an answer.";
  return jsonResponse({ message, messageFormat: "markdown", matches });
}, "handleChat");
var handleSupportCase = /* @__PURE__ */ __name(async (request, env, action, caseId) => {
  ...
  if (action === "create") {
    ...
    await db.prepare(`INSERT INTO support_cases (id, user_id, chat_history, other_context, created_at) VALUES (?, ?, ?, ?, datetime('now'))`).bind(id, userId, JSON.stringify(chatHistory), otherContext ? JSON.stringify(otherContext) : null).run();
    return jsonResponse({ caseId: id, caseUrl: `/support/case/${id}`, prefilledFields: { userId, chatHistory, otherContext } });
  }
  if (action === "feedback") {
    const { caseId: caseId2, rating, comments } = await request.json();
    if (!caseId2 || typeof rating !== "number" || rating < 1 || rating > 5) {
      return errorResponse("Missing or invalid caseId or rating", null, 400);
    }
    await db.prepare(`INSERT INTO support_feedback (case_id, rating, comments, created_at) VALUES (?, ?, ?, datetime('now'))`).bind(caseId2, rating, comments || null).run();
    return jsonResponse({ ok: true });
  }
  if (action === "get" && caseId) {
    const result = await db.prepare(`SELECT id, user_id, chat_history, other_context, created_at FROM support_cases WHERE id = ?`).bind(caseId).first();
...
var handleHelpForm = /* @__PURE__ */ __name(async (request, env) => {
