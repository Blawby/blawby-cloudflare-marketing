...
// .wrangler/tmp/bundle-SOgKnI/checked-fetch.js
...
function extractPricingInfo(contextText) {
  ...
  const monthlyMatch = contextText.match(/\$([0-9]+(?:\.[0-9]{2})?)\s*\/\s*month\s*\/\s*user/i) || contextText.match(/\$([0-9]+(?:\.[0-9]{2})?)\s*per\s*month\s*per\s*user/i) || contextText.match(/\$([0-9]+(?:\.[0-9]{2})?)\s*per\s*month.*user/i);
  if (monthlyMatch) monthlyFee = `$${monthlyMatch[1]} per user per month`;
  const cardMatch = contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*\+\s*([0-9]+¢|\$[0-9]+(?:\.[0-9]{2})?)\s*per.*card/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*\+\s*([0-9]+¢|\$[0-9]+(?:\.[0-9]{2})?)\s*per.*transaction/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*\+\s*([0-9]+¢|\$[0-9]+(?:\.[0-9]{2})?)/i);
  if (cardMatch) cardFee = `${cardMatch[1]}% + ${cardMatch[2]} per card transaction`;
  const achMatch = contextText.match(/([0-9]+(?:\.[0-9]+)?)%.*ACH.*\(\$([0-9]+) cap\)/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*ACH\s*Direct\s*Debit\s*\(\$([0-9]+)\s*cap\)/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*ACH.*\(\$([0-9]+)\)/i);
  if (achMatch) achFee = `${achMatch[1]}% per ACH (max $${achMatch[2]})`;
  const platformMatch = contextText.match(/([0-9]+(?:\.[0-9]+)?)%.*platform fee/i) || contextText.match(/additional\s*([0-9]+(?:\.[0-9]+)?)%\s*fee/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*platform/i);
  if (platformMatch) platformFee = `${platformMatch[1]}% platform fee (billed monthly)`;
  const invoiceMatch = contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*per\s*paid\s*invoice/i) || contextText.match(/([0-9]+(?:\.[0-9]+)?)%\s*per.*invoice/i);
  if (invoiceMatch) invoiceFee = `${invoiceMatch[1]}% per paid invoice`;
  const chargebackMatch = contextText.match(/\$([0-9]+) fee for disputed payments.*chargebacks?/i) || contextText.match(/\$([0-9]+)\s*fee.*chargeback/i) || contextText.match(/\$([0-9]+).*chargeback/i);
...
// .wrangler/tmp/bundle-SOgKnI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SOgKnI/middleware-loader.entry.ts
