...
import Maestro from "react-pay-icons/lib/Maestro";
...
export const PAYMENT_METHODS: PaymentMethod[] = [
    { title: "Visa", component: <Visa width={42} height={24} /> },
    { title: "Mastercard", component: <Mastercard width={85} height={24} /> },
    { title: "Maestro", component: <Maestro width={72} height={24} /> },
    { title: "American Express", component: <div dangerouslySetInnerHTML={{ __html: `<svg
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      {PAYMENT_METHODS.map((method) => (
        <div key={method.title} className="flex flex-col items-center">
          <div aria-label={method.title}>
            {method.component}
          </div>
          <span className="mt-2 text-sm text-center">{method.title}</span>
        </div>
      ))}
    </div>
  );
