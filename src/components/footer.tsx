...
export function Footer() {
  return (
    <footer className="border-t border-gray-950/10 bg-white/90 py-8 text-sm body-text dark:border-white/10 dark:bg-gray-950/90">
      <div className="mx-auto flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 xl:ml-[var(--container-2xs)] xl:max-w-none xl:px-12">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline body-text">&copy; {new Date().getFullYear()} Blawby</span>
        </div>
        ...
        <span className="sm:hidden body-text">&copy; {new Date().getFullYear()} Blawby</span>
      </div>
    </footer>
  );
} 