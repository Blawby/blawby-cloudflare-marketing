...
2. Processing fees (via Stripe) and Blawby's platform fee are not deducted from that payment.
...
- Stripe processes the transaction and calculates fees (e.g., $72.80).
- Blawby charges a 1% platform fee ($25.00).
- Your saved card is charged $97.80 total.
...
Blawby uses Stripe Connect for secure payments infrastructure. Stripe's reliability, PCI compliance, and reporting capabilities support a clean flow of funds with detailed tracking.
