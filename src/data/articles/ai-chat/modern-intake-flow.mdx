...
canonicalUrl: "https://blawby.com/articles/modern-intake-flow"
author: "Team Blawby"
...
// IMPORTANT: Obtain user consent before persisting any session data to localStorage
// Privacy compliance required for GDPR/CCPA
blawby('init', {
  practiceId: 'YOUR_PRACTICE_ID',
  anonymousMode: true,
  sessionTimeout: 30 * 60 * 1000, // 30 minutes
  persistentStorage: 'localStorage' // Alternative: 'memory' for no persistence
});

// Stored data in localStorage (for user disclosure):
// - sessionId: Anonymous session identifier
// - conversationHistory: Chat messages during session
// - userPreferences: Widget settings and preferences
// - sessionTimestamp: When session was created
// - lastActivity: Last user interaction time

// Data retention policy:
// - Automatic purge after sessionTimeout (30 minutes of inactivity)
// - Maximum retention: 24 hours for anonymous sessions
// - User can clear data via browser localStorage clearing

// Opt-out alternatives:
// persistentStorage: 'memory' // No localStorage persistence
// persistentStorage: null // Disable all storage
```
...
```javascript
// Generate or retrieve session ID before use
const sessionId = crypto.randomUUID() || session.id || Date.now().toString();
...
// sessionId should be defined from previous tracking or generated here
blawby('authenticate', {
...
```javascript
// sessionId should be available from previous tracking or generate here
...
```javascript
// Use current date and defined sessionId
const currentDate = new Date().toISOString().split('T')[0];
blawby('retarget', {
  anonymousId: 'anonymous_' + (sessionId || crypto.randomUUID()),
